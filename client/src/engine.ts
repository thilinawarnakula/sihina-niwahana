...
/* ---------- browser orchestrator (Netlify static mode) ----------
 * Fetches through the /api/page proxy function; parallel across sites,
 * sequential + politeness delay within each site (mirrors the NestJS
 * SearchService). This file is a copy of server/src/search/engine.ts
 * plus this block — keep them in sync. */

const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

export async function runSearchInBrowser(
  profile: Profile,
  fetchPage: (url: string) => Promise<string>,
  onProgress?: (msg: string) => void,
) {
  const { tasks, errors } = buildTasks(profile);
  const coverage: Record<string, { searched: string[]; count: number; errors: string[] }> = {};
  ...
  for (const t of tasks) groups.set(t.name, [...(groups.get(t.name) || []), t]);

  const all: Listing[] = [];
  let done = 0;
  await Promise.all([...groups.values()].map(async (group) => {
    for (const t of group) {
      let body: string | null = null, used = t.url;
      try {
        body = await fetchPage(t.url);
      } catch (e: any) {
        if (t.fallbackUrl) {
          try { body = await fetchPage((used = t.fallbackUrl)); }
          catch (e2: any) { cov(t.name).errors.push(`${t.source}: ${e2.message}`); }
      ...
      if (onProgress) onProgress(`Searching… ${done}/${tasks.length} pages`);
      await sleep(1500); // politeness gap per host
    }
  }));
